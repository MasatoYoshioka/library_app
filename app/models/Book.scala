  ...
  def apply(nameString: String,
    ...
    descriptionString: Option[String]): Book = {
      ...
      new Book(
        ...
        publishedDate = 
          if (publishedDateLocal.isEmpty) None
          else publishedDateLocal.map(p => PublishedDate(p)),
        description =
          if (descriptionString.isEmpty) None
          else descriptionString.map(Description)
        )
    }
